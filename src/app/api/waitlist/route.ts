...
export async function POST(request: NextRequest) {
  try {
    ...
    // Insert and get id + referral_code (generated by Supabase default)
    ...
    // Increment referral_count on the referrer, and grant early_access if >= 3
    if (referredBy) {
    ...
    await resend.emails.send({
      ...
      html: `<p style="font-family: monospace; font-size: 14px;">nuevo usuario en noüs.es</p><p style="font-family: monospace; font-size: 14px;"><strong>#${userNumber}</strong> — ${normalizedEmail}${referredBy ? `<br>referido por: ${referredBy}` : ''}</p>`,
    })
